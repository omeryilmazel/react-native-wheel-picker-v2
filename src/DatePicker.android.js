 ...
 * */
 ...
 export default class DatePicker extends React.Component<Props, State> {
   constructor(props: Props) {
     super(props)
     const { startDate, minutes, format, daysCount } = props
     ...
     const formatedDayAsSelectedDate = formatDatePickertAsTimeFormat(selectedDate, format ? format : DEFAULT_DATE_PICKER_FORMAT);
     const formattedDateArray = pickerDateArray(startDate, daysCount, format);
     const initDayInex = formattedDateArray.findIndex(item => item === String(formatedDayAsSelectedDate));
     const initHourInex = this.props.format24
       ? time24format
       : Number(time12format[0]) - 1
     const minutesData = minutes ? minutes : getMinutesArray();
     const initMinuteInex =  Number(selectedDate.getMinutes());
     ...
     if (data[position] === 'Today') {
       ...
       selectedDate = increaseDateByDays(
         ...
         this.props.startDate
           ? daysAfterSelectedDate + (position + 1)
           : position
       )
     }
   ...
   onMinuteSelected = (position: number) => {
     ...
     const data = minutes || getMinutesArray()
